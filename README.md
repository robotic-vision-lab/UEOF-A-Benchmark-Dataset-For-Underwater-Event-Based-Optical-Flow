...
Some benchmark configs in this repository use `s1` to `s5` for the deep split.
If your downloaded dataset uses `scene1` to `scene5`, update the paths or
create symlinks before running those configs.
...
| `depth/` | `.png` | Depth maps, deep-water environment only |
...
The `depth/` directory is present only in deep-water environment scenes.
`velocities.npz` is omitted for static scenes, currently `shallow/scene1` and
`shallow/scene5`.

All image and flow files use zero-padded five-digit names starting from
`00000`, such as `00000.png` and `00001.flo`.

## Data Specifications

### Event Data

Events are stored in `events.h5` under an HDF5 dataset named `events` with
shape `(N, 4)`. Each row follows `(t, x, y, p)`:
...
- Deep-water environment: `1280x720`
- Shallow-water environment: `960x540`
...
All timestamp files use milliseconds. Deep-water environment timestamps are
integers, while shallow-water environment timestamps are floating point.
...
Optical flow is computed between consecutive frames. For `N` images, there are
typically `N - 1` flow files, where flow file `i` represents the flow from
...
The velocity is saved as uncompressed NumPy `.npz` archives in
`velocities.npz`. This file contains linear and angular velocities and is
omitted for static scenes.

The deep-water environment scenes contain:
...
The shallow-water environment scenes contain:
...
The shallow-water scenes do not include `pos` and `quat` arrays.

### Depth Data

Depth maps are provided for all deep-water environment scenes in `depth/`. Each
file is a 16-bit grayscale PNG named to match its corresponding RGB frame, such
as `00000.png`.

Encoding follows the [KITTI depth
standard](http://www.cvlibs.net/datasets/kitti/): each pixel stores the
distance from the camera to the corresponding surface element, linearly mapped
to the 16-bit unsigned integer range `[0, 2^{16} - 1]`.

The depth resolution `r` is given by:

$$r = \frac{d_{\text{end}} - d_{\text{start}}}{2^{16} - 1}$$

With maximum range `d_end = 25 m` and `d_start = 0 m`, this yields a resolution
of about `0.381 mm/count`.

To recover depth in meters from a raw pixel value `v`:

$$d = v \times \frac{25}{65535}$$

Pixel value `65535` indicates invalid or occluded depth. Mask this value before
converting pixel values to meters to avoid false maximum-range readings.
...
Static scenes, `shallow/scene1` and `shallow/scene5`, have no ego-motion and
therefore no velocity data. The shallow-water sequences contain an extra empty
ending flow file.

## Benchmarking Code

The `benchmarking/` directory contains repository-specific code for the methods
evaluated in the paper. The benchmark implementations are organized so they can
be copied into their corresponding upstream projects with minimal changes.
...
The benchmark configs are written with local path placeholders. Before running
them, update the dataset root and output directories to match your machine.
...
The configs under `benchmarking/event_based_optical_flow/config/` define one
sequence per YAML file.
...
Each script launches `experiments.e16` with the appropriate sequence name,
event count, and optimization settings.
...
Edit the dataset root, checkpoint path, and output directory before running
inference.

### DSEC Style Evaluation

Use `benchmarking_scripts/DSEC-format_benchmark.py` to score predicted flow
PNGs against `.flo` ground truth:
...
The dataset generation utilities under `dataset_scripts/` are split into two
stages:

1. `dataset_scripts/v2e/`
   - shell scripts for generating event streams from rendered videos with `v2e`
   - separate configs for the shallow-water and deep-water dataset splits
...
The scripts assume the data is already organized on disk. The repository does
not prescribe a single raw data layout, but the benchmark configs use example
roots such as `/datasets/shallow` and `/datasets/deep`.
