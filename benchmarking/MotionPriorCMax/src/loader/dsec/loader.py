...
class DatasetProvider:
    def __init__(
        ...
        seq_names = (
            TRAIN_SEQS
            if split == "train"
            else VAL_SEQS if split == "val" else []
        )
...
class Sequence(Dataset):
    def __init__(
        ...
        self.height, self.width = 540, 960  # output dimensions
        ...
        # allow rectify map to be optional
        self.events_h5 = None
        self.event_slicer = None
        self._open_events_only(seq_path)
        # self.rectify_ev_map = None
        # self._initialize_event_data(seq_path)
        # self.x_map = None
        # self.y_map = None
        # self._open_event_and_rectify_maps(seq_path)
    ...
    def _initialize_event_data(self, seq_path: Path):
        ...
        h5f = h5py.File(seq_path / "events.h5", "r")
        self.events_h5 = h5f

        # rectify_map = h5py.File(ev_dir / 'rectify_map.h5', 'r')['rectify_map'][()]
        rectify_path = seq_path / "rectify_map.h5"
        if rectify_path.exists():
            with h5py.File(rectify_path, "r") as rf:
                self.rectify_ev_map = rf["rectify_map"][()]
        else:
            self.rectify_ev_map = None

        return EventSlicer(h5f), self.rectify_ev_map

    def _open_events_only(self, seq_path: Path):
        events_file = seq_path / "events.h5"
        if not events_file.exists():
            raise FileNotFoundError(f"No events.h5 at {events_file}")

        self.events_h5 = h5py.File(events_file, "r")
        self.event_slicer = EventSlicer(self.events_h5)

    def _load_training_data(self, seq_path: Path):
        """Load timestamps and paths for training phase."""
        timestamps_images = np.loadtxt(
            seq_path / "images/timestamps.txt", dtype="int64"
        )
        ...
        self.timestamps_flow, self.indices = (
            self.timestamps_flow[keep_i],
            self.indices[keep_i],
        )
        self.paths_to_forward_flow = [
            seq_path / "flow/forward" / f"{str(i).zfill(6)}.png"
            for i in self.indices
        ]

    def _load_validation_data(self, seq_path: Path):
        """Load timestamps and paths for validation phase."""
        self.timestamps_flow = np.loadtxt(
            seq_path / "flow/forward_timestamps.txt",
            delimiter=",",
            skiprows=1,
            dtype="int64",
        )
        ...
        flow_files = [
            f
            for f, keep in zip(
                sorted(os.listdir(seq_path / "flow/forward")), keep_i
            )
            if keep
        ]
        self.paths_to_forward_flow = [
            seq_path / "flow/forward" / f for f in flow_files
        ]
    ...
    def _load_test_data(self, seq_path: Path, timestamp_path: str):
        ...
        self.timestamps_flow = np.stack(
            (
                df["from_timestamp_us"].to_numpy(),
                df["to_timestamp_us"].to_numpy(),
            ),
            axis=1,
        )
    ...
    def _close_h5_file(self):
        if self.events_h5 is not None:
            try:
                self.events_h5.close()
            except:
                pass
    ...
    def get_data_sample(self, index, flip=None):
        ...
        t = (event_data["t"] - event_data["t"].min()) / (
            event_data["t"].max() - event_data["t"].min()
        )
        bin_indices = np.clip(np.searchsorted(self.t_bins, t) - 1, 0, None)
        events = np.column_stack(
            (y_rect, x_rect, t, event_data["p"], bin_indices)
        )

        mask = (
            (0 <= events[:, 0])
            & (events[:, 0] < self.height)
            & (0 <= events[:, 1])
            & (events[:, 1] < self.width)
        )
        ...
        output["voxel"] = self.events_to_voxel_grid(
            event_data["p"], t, x_rect, y_rect
        )

        if self.paths_to_forward_flow is not None:
            flow_path = Path(self.paths_to_forward_flow[index])
            if flow_path.exists():
                flow_16bit = imageio.imread(flow_path, format="PNG-FI").astype(
                    np.float32
                )
                flow = np.zeros((2, self.height, self.width), dtype=np.float32)
                flow[0] = (flow_16bit[..., 1] - 2**15) / 128.0
                flow[1] = (flow_16bit[..., 0] - 2**15) / 128.0
    ...
    def rectify_events(self, x, y):
        ...
        # by default use an identity mapping for rectify map
        return np.stack((x, y), axis=1)

        # if self.rectify_ev_map is not None:
    ...
    def get_disparity_map(filepath: Path):
        ...
        return (
            cv2.imread(str(filepath), cv2.IMREAD_ANYDEPTH).astype("float32")
            / 256
        )
...
class EventSlicer:
    ...
    def get_events(
        ...
        time_array_conservative = np.asarray(
            self.events["t"][t_start_ms_idx:t_end_ms_idx], dtype="int64"
        )
        ...
        events["t"] = (
            time_array_conservative[idx_start_offset:idx_end_offset]
            + self.t_offset
        )
    ...
    @staticmethod
    def get_conservative_window_ms(
        ts_start_us: int, ts_end_us
    ) -> Tuple[int, int]:
...
def sequence_collate_fn(batch):
    ...
    event_types = (
        ("events",) if "events" in batch[0] else ("pos_events", "neg_events")
    )
    ...
    for sample in batch:
        if len(event_types) == 1:
            ...
            pos_padded = pad_events(
                sample["pos_events"], max_events["pos_events"]
            )
            neg_padded = pad_events(
                sample["neg_events"], max_events["neg_events"]
            )
    ...
    if all_have_gt_flow:
        batched_data["forward_flow"] = torch.stack(
            batched_data["forward_flow"], dim=0
        )
        batched_data["flow_valid"] = torch.stack(
            batched_data["flow_valid"], dim=0
        )
