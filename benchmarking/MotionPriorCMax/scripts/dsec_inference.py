...
def propagate_config(config):
    ...
    config["model"].update(
        {"image_shape": image_shape, "num_bins": config["common"]["num_bins"]}
    )
...
def scale_optical_flow(flow, max_flow_magnitude):
    ...
    u[exceed_indices] = (
        u[exceed_indices] / magnitude[exceed_indices]
    ) * max_flow_magnitude
    v[exceed_indices] = (
        v[exceed_indices] / magnitude[exceed_indices]
    ) * max_flow_magnitude
...
def save_flow(file_path: Path, flow: np.ndarray):
    ...
    flow_16bit[..., 1] = (flow[0] * 128 + 2**15).astype(
        np.uint16
    )  # y-component
    flow_16bit[..., 0] = (flow[1] * 128 + 2**15).astype(
        np.uint16
    )  # x-component
...
def load_model(config, device):
    ...
    model.load_state_dict(
        torch.load(
            config["model"]["ckpt_path"], map_location=device, weights_only=True
        )
    )
...
def process_sequence(
    seq, config, device, model, mask, run_out_dir, timestamp_dir, dataset_dir
):
    ...
    dataset = Sequence(
        seq_dir,  # folder must contain events.h5 and rectify_map.h5 directly
        "test",
        config["data"]["num_bins"],
        timestamp_path=timestamp_path,
        # rectify map not required
        polarity_aware_batching=config["data"]["polarity_aware_batching"],
    )
    loader = DataLoader(
        dataset, batch_size=1, shuffle=False, collate_fn=sequence_collate_fn
    )
...
def process_sample(sample, device, model, mask, flow_out_dir, config):
    ...
    coeffs, pixel_positions, _ = utils.coeffs_grid_to_list(
        coeff_grid, mask, num_coeffs=1
    )

    # Compute trajectory and optical flow
    traj_at_t0 = utils.compute_basis(
        coeffs,
        torch.zeros(1, device=device),
        config["model"]["num_basis"],
        config["model"]["basis_type"],
    )
    traj_at_t1 = utils.compute_basis(
        coeffs,
    ...
    flow, _ = utils.dense_flow_from_traj(
        traj_flow[..., 0, :],
        pixel_positions,
        config["model"]["patch_size"],
        config["model"]["image_shape"],
    )
...
def main():
    ...
    mask = utils.get_optical_flow_tile_mask(
        config["model"]["image_shape"], config["model"]["patch_size"]
    )

    timestamp_dir = (
        Path(__file__).resolve().parent / "../config/misc/dsec_test_timestamps"
    )
    test_seqs = ["scene1"]
